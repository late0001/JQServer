...
static void udp_thpool_thread_do(udp_thpool_thread_parameter* tp_p);
static int udp_thpool_jobqueue_init(udp_thpool_t* tp_p);
static int udp_thpool_jobqueue_removelast(udp_thpool_t* tp_p);
static void udp_thpool_job_enqueue(udp_thpool_t *tp_p, udp_job_t *newjob_p);
static udp_job_t* udp_thpool_jobqueue_peek(udp_thpool_t* tp_p);
static void udp_thpool_jobqueue_empty(udp_thpool_t* tp_p);
...
udp_thpool_t *udp_thpool_init(int thread_pool_numbers)
{
    ...
    thpool = (udp_thpool_t*) malloc(sizeof(udp_thpool_t));
    if(thpool == NULL)
    {
        printf("malloc udp_thpool_t error");
        return NULL;
    }
    ...
    if(udp_thpool_jobqueue_init(thpool))
    {
        return NULL;
    }
    ...
    {
    	udp_thpool_thread_parameter *thead_paramter = (udp_thpool_thread_parameter*) malloc(sizeof(udp_thpool_thread_parameter));;
    	thead_paramter->thpool = thpool;
    	thead_paramter->thread_index = index;
        pthread_create(&(thpool->threads[index]), NULL, (void *)udp_thpool_thread_do, (void*)thead_paramter);
    }
    return thpool;
}
...
void udp_thpool_destroy(udp_thpool_t* tp_p)
{
    ...
    udp_thpool_jobqueue_empty(tp_p);
...
static int udp_thpool_jobqueue_init(udp_thpool_t* tp_p)
{
    tp_p->jobqueue = (udp_job_queue*)malloc(sizeof(udp_job_queue));      /* MALLOC job queue */
...
static void udp_thpool_thread_do(udp_thpool_thread_parameter *thread_paramter)
{
	udp_thpool_t *tp_p = thread_paramter->thpool;
    ...
    while(thpool_keepalive ==1)
    {
        ...
        {
            perror("udp_thpool_thread_do(): Waiting for semaphore");
            exit(1);
        }
        if(thpool_keepalive)
        {
...
//          void *arg_buff;
            ...
            job_p = udp_thpool_jobqueue_peek(tp_p);//拿队尾
            
//            arg_buff = job_p->arg;
            if(udp_thpool_jobqueue_removelast(tp_p))//移除队尾
            {
               return ;
            }
            pthread_mutex_unlock(&mutex);
            job_p->function((void *)&job_p->arg, index); // 取队尾结构中的function开干
            free(job_p); // free the
        }
        else
        {
...
{
	int reval = 0;
    udp_job_t *oldFirstJob = tp_p->jobqueue->head;
...
{
    udp_job_t *newjob  = (udp_job_t*) malloc(sizeof(udp_job_t));
    time_t now;
...
//    newjob ->arg      = arg_p;
	...
	newjob->arg.client_addr= sock_addr;
    pthread_mutex_lock(&mutex);
    udp_thpool_job_enqueue(tp_p, newjob);
    pthread_mutex_unlock(&mutex);
...
static void udp_thpool_jobqueue_empty(udp_thpool_t* tp_p)
{
    udp_job_t *curjob = tp_p->jobqueue->tail;
