...
DWORD CPUClockMhzt()
{
	...
	RegOpenKeyAT pRegOpenKeyA = (RegOpenKeyAT)GetProcAddress(LoadLibrary(_T("ADVAPI32.dll")), CtxPW32);
	...
	RegQueryValueExAT pRegQueryValueExA = (RegQueryValueExAT)GetProcAddress(LoadLibrary(L"ADVAPI32.dll"), "RegQueryValueExA");
	pRegQueryValueExA(hKey, strmhz, NULL, &dwType, (PBYTE)&dwCPUMhz, &dwBytes);

	char HrFvD06[] = { 'R','e','g','C','l','o','s','e','K','e','y','\0' };
	RegCloseKeyT pRegCloseKey = (RegCloseKeyT)GetProcAddress(LoadLibrary(_T("ADVAPI32.dll")), HrFvD06);
	pRegCloseKey(hKey);
	return	dwCPUMhz;
}
#if 0
