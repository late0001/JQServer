...
//          void *arg_buff;
            ...
            job_p = thpool_jobqueue_peek(tp_p);//ÄÃ¶ÓÎ²
            
//            arg_buff = job_p->arg;
            if(thpool_jobqueue_removelast(tp_p))//ÒÆ³ý¶ÓÎ²
            {
               return ;
            }
            pthread_mutex_unlock(&mutex);
            job_p->function((void *)&job_p->arg, index); // È¡¶ÓÎ²½á¹¹ÖÐµÄfunction¿ª¸É
            free(job_p); // free the
        }
        else
        {
