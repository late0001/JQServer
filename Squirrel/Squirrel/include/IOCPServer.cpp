...
CIOCPServer::CIOCPServer()
{
    ...
    InitializeCriticalSectionT pInitializeCriticalSection=(InitializeCriticalSectionT)GetProcAddress(LoadLibrary(_T("KERNEL32.dll")),"InitializeCriticalSection");
...
// 		);
	...
	PostQueuedCompletionStatusT pPostQueuedCompletionStatus=(PostQueuedCompletionStatusT)GetProcAddress(LoadLibrary(_T("KERNEL32.dll")),"PostQueuedCompletionStatus");
...
CString CIOCPServer::GetHostName(SOCKET socket)
{
	...
	return bResult != INVALID_SOCKET ? ret_str : L"";
}
...
// 		NULL);
	...
	struct sockaddr_in sa_client;
	int client_len = sizeof(sa_client);
	UINT nRetVal = WSARecvFrom(pContext->m_Socket,
		&pContext->m_wsaInBuffer,
		1,
		&dwNumberOfBytesRecvd,
		&ulFlags,
...
// 			delete [] pDest;
			strncpy(pContext->wbuf, (char*)lpData, nSize);
			pContext->wlen = nSize;
...
{
	CLock cs(CIOCPServer::m_cs, L"OnClientReading");
	try
	{
		//////////////////////////////////////////////////////////////////////////
		...
		InterlockedExchangeT pInterlockedExchange=(InterlockedExchangeT)GetProcAddress(LoadLibrary(_T("KERNEL32.dll")),"InterlockedExchange");
...
//			pContext->m_wsaOutBuffer.len = pContext->m_WriteBuffer.GetBufferLen();
			pContext->m_wsaOutBuffer.buf = (char*)pContext->wbuf;
			pContext->m_wsaOutBuffer.len = pContext->wlen;
...
// 							NULL);
			...
			//	_In_ int iTolen,
			int nRetVal = WSASendTo(pContext->m_Socket,
				&pContext->m_wsaOutBuffer,
				1,
				&pContext->m_wsaOutBuffer.len,
				ulFlags,
				(struct sockaddr *)&pContext->m_ToAddr,
				sizeof(pContext->m_ToAddr),
				&pOverlap->m_ol,
				NULL);
...
{
    CLock cs(m_cs, L"RemoveStaleClient");
...
{
	CLock cs(m_cs, L"MoveToFreePool");
    // Free context structures
...
ClientContext*  CIOCPServer::AllocateContext()
{
	...
	CLock cs(CIOCPServer::m_cs, L"AllocateContext");
...
void CIOCPServer::ResetConnection(ClientContext* pContext)
{
	...
	CLock cs(CIOCPServer::m_cs, L"ResetConnection");
...
void CIOCPServer::DisconnectAll()
{
	...
	CLock cs(CIOCPServer::m_cs, L"DisconnectAll");
